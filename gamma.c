              ...
              int forkelem)
{
    ...
    if (mutex_init(&c.mtx_al, NULL) != 0)
        goto f1;
    if ((c.pool = calloc(maxthreads, sizeof(struct qsort))) == NULL)
        goto f2;
    for (islot = 0; islot < maxthreads; islot++) {
        qs = &c.pool[islot];
        if (mutex_init(&qs->mtx_st, NULL) != 0)
            goto f3;
        if (cond_init(&qs->cond_st, NULL) != 0) {
            verify(mutex_destroy(&qs->mtx_st));
            goto f3;
        }
        qs->st = ts_idle;
        qs->common = &c;
        if (pthread_create(&qs->id, NULL, qsort_thread, qs) != 0) {
            verify(mutex_destroy(&qs->mtx_st));
            verify(cond_destroy(&qs->cond_st));
            goto f3;
        }
    }
...
nevermind:
    ...
    if (nl > c->forkelem && nr > c->forkelem &&
        ...
        qs2->a = a;
        qs2->n = nl;
        verify(cond_signal(&qs2->cond_st));
...
again:
    ...
    verify(mutex_lock(&qs->mtx_st));
    while (qs->st == ts_idle)
    	verify(cond_wait(&qs->cond_st, &qs->mtx_st)); //HHHH
    verify(mutex_unlock(&qs->mtx_st));
    if (qs->st == ts_term) {
        return NULL;
    }
    ...
    if (c->idlethreads == c->nthreads) {
        for (i = 0; i < c->nthreads; i++) {
            ...
            qs2->st = ts_term;
    	    verify(cond_signal(&qs2->cond_st)); //JJJJ
            verify(mutex_unlock(&qs2->mtx_st));
        }
        verify(mutex_unlock(&c->mtx_al));
        return NULL;
